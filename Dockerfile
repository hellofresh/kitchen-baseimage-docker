...
# Install python
RUN apt-get update && \
    apt-get install --no-install-recommends -y software-properties-common python

# Install openssh and curl for test-kitchen
RUN apt-get install --no-install-recommends -y openssh-server curl lsb-release
...
ENV busser_plugins      busser-serverspec
